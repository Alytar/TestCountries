...
internal fun <T : RealmModel> queryFirstAsync(callback: (T?) -> Unit, javaClass: Class<T>) {
    onLooperThread {
        ...
        RealmObject.addChangeListener(result) { value ->
            callback(if (RealmObject.isValid(value)) realm.copyFromRealm(value) else null)
...
internal fun <T : RealmModel> queryAllAsync(callback: (List<T>) -> Unit, javaClass: Class<T>) {
    onLooperThread {
        ...
        result.addChangeListener { value ->
            callback(realm.copyFromRealm(value))
...
inline fun <reified T : RealmModel> queryAsync(noinline query: Query<T>, noinline callback: (List<T>) -> Unit) =
