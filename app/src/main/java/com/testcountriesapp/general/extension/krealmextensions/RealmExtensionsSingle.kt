 ...
 */
fun <T : RealmModel> T.querySortedAsSingle(fieldName: List<String>, order: List<Sort>, query: Query<T>? = null) =
 ...
 * Query for sorted entities and observe changes returning a Single.
 */
fun <T : RealmModel> T.querySortedAsSingle(fieldName: String, order: Sort, query: Query<T>? = null) =
...
internal fun <T : RealmModel> performSingleQuery(
    ...
    return Single.create<List<T>> { emitter ->
        ...
        result.addChangeListener { value ->
            emitter.onSuccess(realm.copyFromRealm(value))
        }
